...
<title>🚙 Vehicle 2 — V2V archive</title>
<meta name="description" content="Archived V2V civilian vehicle node 2 for the V2X Connect system.">
...
:root{--ac:#aa44ff;--bg:#06040f;--card:#0d0a1e;--border:rgba(255,255,255,.06);--dim:#334}
body{font-family:'Inter',sans-serif;background:var(--bg);color:#ccd0e8;min-height:100vh;user-select:none}
...
.hdr{padding:12px 15px 8px;text-align:center;background:linear-gradient(180deg,#06040e,#090418);border-bottom:1px solid rgba(170,68,255,.12);position:relative}
.hdr h1{font-family:'Orbitron';font-size:1rem;color:var(--ac);letter-spacing:3px;margin-bottom:2px;text-shadow:0 0 22px rgba(170,68,255,.4)}
.hdr p{font-size:.6rem;color:var(--dim)}
.logout-chip{position:absolute;right:12px;top:50%;transform:translateY(-50%);padding:4px 10px;border-radius:8px;border:1px solid rgba(170,68,255,.25);background:rgba(170,68,255,.07);color:#c8f;font-size:.58rem;font-weight:700;cursor:pointer;font-family:'Inter',sans-serif}
...
.dirarr{background:rgba(0,0,0,.35);border-radius:14px;padding:14px;margin:12px 0 4px;border:2px solid rgba(255,30,30,.3);display:none}
.darrow{font-size:4rem;line-height:1;margin:4px 0;animation:pulse .6s ease-in-out infinite alternate}
@keyframes pulse{to{transform:scale(1.14)}}
.darrow-t{font-family:'Orbitron';font-size:.9rem;letter-spacing:2px;font-weight:900;margin-bottom:3px}
...
.pbar{height:10px;background:rgba(255,255,255,.05);border-radius:5px;overflow:hidden}
.pbar-fill{height:100%;border-radius:5px;background:linear-gradient(90deg,#00dd66,#ffaa00,#ff3344);width:0%;transition:.8s}
...
.sech span:last-child{font-size:.58rem;padding:2px 8px;border-radius:8px;background:rgba(170,68,255,.1);color:#c8f;border:1px solid rgba(170,68,255,.2)}
...
.li.s{border-color:#aa44ff;color:#c8f}.li.a{border-color:#f23;color:#f88}
.li.v{border-color:#0d6;color:#8fa}.li.w{border-color:#fa0;color:#fc6}
.foot{text-align:center;padding:10px;font-size:.54rem;color:#1a2233}
.overlay{position:fixed;inset:0;background:rgba(6,4,15,.97);display:none;flex-direction:column;align-items:center;justify-content:center;z-index:999;padding:30px;text-align:center}
...
<div class="zone safe" id="zone">
  <div class="zone-in">
    ...
    <div class="pbar-wrap">
      <div class="pbar-lbls"><span>Far &gt;150m</span><span id="dlbl">—</span><span>⚠️ 25m</span></div>
...
<div class="sec">
  ...
  <div class="grid2">
    ...
    <div class="gc"><div class="lb">DIST TO EV</div><div class="vl" id="dev">—</div></div>
  </div>
</div>
...
<div class="foot">🚙 Vehicle 2 v5.0 · Real GPS · V2V Detection</div>

<div class="overlay" id="gpserr">
  <h2>📵 GPS REQUIRED</h2>
  <p>Enable location permissions. This node needs real GPS to work.</p>
...
(async () => {
  const ok = await requireAuth(); // no strict role check
  if (!ok) return;
  const s = getSession();
...
DB.emergency.on('value',s=>{
  ...
  const fp=Math.max(0,Math.min(100,(1-dist/200)*100));
  ...
  document.getElementById('evdist').textContent=dist+'m '+(dist<=RANGE_V2V?'🔴 DANGER':dist<=RANGE_V2I?'🟡 NEAR':'🟢 Safe');
...
function setDanger(dist,side,ico){
  ...
  if(!hasAlert){addLog('a','🚨 EV at '+dist+'m — MOVE '+side);logEvent('v2v','V2 alert',{dist,side});hasAlert=true;}
}
...
function doYield(){
  ...
  logEvent('v2v','V2 yielded',{lat:myLat,lng:myLng});
...
addLog('s','🚙 Vehicle 2 v6.0 — GPS starting');
</script>
</body>
</html>
