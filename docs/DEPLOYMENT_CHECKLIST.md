...
- [ ] `package.json` has Node 20+ runtime (`engines.node`)
- [ ] `vercel.json` exists with Next.js framework
- [ ] `next.config.mjs` contains `/legacy/*` rewrites
- [ ] `next.config.mjs` contains headers for `sw.js` and legacy assets
- [ ] Firebase authorized domains include `<project>.vercel.app`
- [ ] Firebase authorized domains include production custom domain (if any)
- [ ] Post-deploy smoke test passes for `/login`, `/control`, `/emergency`, `/legacy/control`
